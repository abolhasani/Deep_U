...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        # (2) Use a word embedding layer to transform the words in captions
        embedded_captions_in, cache_embed_in = word_embedding_forward(captions_in, W_embed)
        # (3) Use vanilla RNN to process the sequence of input word vectors and produce hidden state
        forward_net = { "rnn": rnn_forward}[self.cell_type]
        h, cache_rnn = forward_net(embedded_captions_in, h0, Wx, Wh, b)
        # (4) Use a (temporal) affine transformation to compute scores over the vocabulary
        ...
        # we begin our gradient check with dout, like previous times
        # backward pass 
        # (4) Compute gradients for the affine transformation layer
        dout, grads["W_vocab"], grads["b_vocab"] = temporal_affine_backward(dout, cache_temporal)
        # (3) Compute gradients for the vanilla RNN
        backward_net = {"rnn": rnn_backward}[self.cell_type]
        ...
        # (1) Compute gradients for the affine transformation to compute the initial hidden state
        dx_proj, grads["W_proj"], grads["b_proj"] = affine_backward(dh0, cache_affine)
        """
        """ 
        ############################################################################
        #                             END OF YOUR CODE                             #
    ...
    def sample(self, features, max_length=30):
        ...
        W_Shape_1, W = W_embed.shape 
        x = np.ones((N, W)) * W_embed[self._start]
        for t in range(max_length):
            next_h = rnn_step_forward(x, h, Wx, Wh, b)[0]  
